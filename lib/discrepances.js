...
function getClassOnlyForSomeOfBuiltIns(variable) {
...
function setPropertyIf(object, key, value, optionalCond) { if(optionalCond || value) { object[key] = value; } }
...
function compareObjects(a, b, opts) {
    ...
    if(opts.unordered) {
        aKeys.forEach(function(key) { 
            setPropertyIf(rv, key, !(key in b) ? {onlyLeft:a[key]} : discrepances(a[key], b[key]));
        });  
        bKeys.forEach(function(key) {
            setPropertyIf(rv, key, !(key in a) ? {onlyRight:b[key]} : discrepances(a[key], b[key]));
        });        
    } else {
        var diffs = [];
        aKeys.forEach(function(key,index) {
            ...
            if(key !== bKey) {
                dif.keys = [key, bKey];
                setPropertyIf(dif, 'values', discrepances(a[key], b[bKey]));
            } else {
                setPropertyIf(dif, 'values', discrepances(a[key], b[key]));
            }
        ...
        if(typeA==='object' && typeB==='object') {
            ...
            if(classA !== classB) {
                ...
                if(classA==='Array') {
                    ...
                    return compareObjects(a, b, opts);
                }
            }
        }
    } else {
